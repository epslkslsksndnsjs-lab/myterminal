...
#### [src/cli.ts](file:///Users/a0000/Desktop/myterminal/src/cli.ts)
...
#### [src/config.ts](file:///Users/a0000/Desktop/myterminal/src/config.ts)
...
#### [src/migration.ts](file:///Users/a0000/Desktop/myterminal/src/migration.ts)
跨目录 `state.json` 与 `history/*.jsonl` 的合并迁移，幂等且按稳定 ID 去重保留。`migrateWorkspaceState`。

### 5.2 运行时与集群层

#### [src/server.ts](file:///Users/a0000/Desktop/myterminal/src/server.ts) — 组合根
...
#### [src/cluster.ts](file:///Users/a0000/Desktop/myterminal/src/cluster.ts)
...
#### [src/cluster-router.ts](file:///Users/a0000/Desktop/myterminal/src/cluster-router.ts)
...
#### [src/control-channel.ts](file:///Users/a0000/Desktop/myterminal/src/control-channel.ts)
`class ControlChannelMonitor`：周期探活 `publicBaseUrl`，恢复后触发 `revalidateAfterResume`；指数退避重连；`classifyControlChannelFailure` 分类失败。

#### [src/instances.ts](file:///Users/a0000/Desktop/myterminal/src/instances.ts)
...
#### [src/runtime-lifecycle.ts](file:///Users/a0000/Desktop/myterminal/src/runtime-lifecycle.ts)
原子写入 `runtime-state.json`：starting/active/revalidating/degraded/shutting_down/stopped。

#### [src/session-resource-manager.ts](file:///Users/a0000/Desktop/myterminal/src/session-resource-manager.ts)
`SessionResourceManager` 单例：agent/session/global 三级资源清理单点注册与统一收口。`disposeAgent(agentId)`/`disposeSession(sessionId)`。

#### [src/session-resources.ts](file:///Users/a0000/Desktop/myterminal/src/session-resources.ts)
macOS 被动锁 helper 的编译/启停；per-session awake-lock 的 arm/disarm/reap。`verifyRuntimeResources()`。

### 5.3 域状态层

#### [src/store.ts](file:///Users/a0000/Desktop/myterminal/src/store.ts) — `MyTerminalStore`
...
#### [src/types.ts](file:///Users/a0000/Desktop/myterminal/src/types.ts)
全部核心类型定义（详见 [§6 关键类型](#61-域模型-types)）。

#### [src/context-projector.ts](file:///Users/a0000/Desktop/myterminal/src/context-projector.ts)
...
#### [src/tui-model.ts](file:///Users/a0000/Desktop/myterminal/src/tui-model.ts)
TUI 纯模型：`logicalSessionGroups`（按 continuesSessionId 链归并到 origin root）、`conversationGroups`、`selectedViewport`。

### 5.4 扩展门面层

#### [src/extensions.ts](file:///Users/a0000/Desktop/myterminal/src/extensions.ts) — `ExtensionService`
...
#### [src/core-tools.ts](file:///Users/a0000/Desktop/myterminal/src/core-tools.ts)
...
#### [src/openapi.ts](file:///Users/a0000/Desktop/myterminal/src/openapi.ts)
...
#### [src/mcp.ts](file:///Users/a0000/Desktop/myterminal/src/mcp.ts) — `MyTerminalMcpTransport`
...
#### [src/mcp-schema.ts](file:///Users/a0000/Desktop/myterminal/src/mcp-schema.ts)
...
#### [src/tool-schemas.ts](file:///Users/a0000/Desktop/myterminal/src/tool-schemas.ts)
`BUILTIN_INPUT_SCHEMAS`：所有内置工具形状单源（core-tools 运行期校验 + mcp 派生 + discover 目录三处共用）；`TASK_POLL_TOOL`。

### 5.5 Skill 系统

#### [src/skills.ts](file:///Users/a0000/Desktop/myterminal/src/skills.ts)
...
#### [src/diff.ts](file:///Users/a0000/Desktop/myterminal/src/diff.ts)
`WorkspaceDiffTracker`：有界 Git diff/status 采集，untracked 文件采样，TUI 周期刷新。Git 可选能力，探测一次后非 Git 目录安全降级；每个子进程有 deadline 与有界输出。

#### [src/security.ts](file:///Users/a0000/Desktop/myterminal/src/security.ts)
...
#### [src/audit-log.ts](file:///Users/a0000/Desktop/myterminal/src/audit-log.ts)
...
#### [src/redact.ts](file:///Users/a0000/Desktop/myterminal/src/redact.ts) — 脱敏单源（ADR-0026）
...
#### [src/update.ts](file:///Users/a0000/Desktop/myterminal/src/update.ts) / [src/update-transaction.ts](file:///Users/a0000/Desktop/myterminal/src/update-transaction.ts)
...
#### [src/continuation.ts](file:///Users/a0000/Desktop/myterminal/src/continuation.ts)
...
#### `MyTerminalRuntime`（[server.ts](file:///Users/a0000/Desktop/myterminal/src/server.ts))
...
#### `ExtensionService`（[extensions.ts](file:///Users/a0000/Desktop/myterminal/src/extensions.ts))
...
#### `MyTerminalStore`（[store.ts](file:///Users/a0000/Desktop/myterminal/src/store.ts))
...
#### `PortClusterRegistry`（[cluster.ts](file:///Users/a0000/Desktop/myterminal/src/cluster.ts))
...
#### `ClusterExtensionRouter`（[cluster-router.ts](file:///Users/a0000/Desktop/myterminal/src/cluster-router.ts))
...
#### 接入层 — [runner.ts](file:///Users/a0000/Desktop/myterminal/src/subagent/runner.ts)
...
#### 执行核心 — [executor.ts](file:///Users/a0000/Desktop/myterminal/src/subagent/executor.ts)
...
#### 工具层 — [tools.ts](file:///Users/a0000/Desktop/myterminal/src/subagent/tools.ts) / [tool-executor.ts](file:///Users/a0000/Desktop/myterminal/src/subagent/tool-executor.ts)
...
#### LLM 适配 — [llm-adapter.ts](file:///Users/a0000/Desktop/myterminal/src/subagent/llm-adapter.ts)
...
#### 隔离 — [worktree.ts](file:///Users/a0000/Desktop/myterminal/src/subagent/worktree.ts)
...
#### 权限 — [permissions.ts](file:///Users/a0000/Desktop/myterminal/src/subagent/permissions.ts)
...
#### 状态/辅助
- [store.ts](file:///Users/a0000/Desktop/myterminal/src/subagent/store.ts)：内存 Map，`createSubagent`/`updateSubagentStatus`（1h 清理定时器）/`addAuditLog`（截断+保留 50）/`countRunning`/`listAllSubagents`。
- [file-state.ts](file:///Users/a0000/Desktop/myterminal/src/subagent/file-state.ts)：`recordFileRead`/`validateEdit`（0 匹配附预览，>1 非 replaceAll 报错）/`applyEdit`。
- [shell-tracker.ts](file:///Users/a0000/Desktop/myterminal/src/subagent/shell-tracker.ts)：`trackShellTask`/`cleanupAgentShellTasks`（杀进程组，2s SIGKILL 兜底）。
- [result-budget.ts](file:///Users/a0000/Desktop/myterminal/src/subagent/result-budget.ts)：`truncateResult`（MAX 50K）/`enforceMessageBudget`（200K 预算，超限按长度降序压成预览并冻结）/`ensureNonEmpty`。
- [token-counter.ts](file:///Users/a0000/Desktop/myterminal/src/subagent/token-counter.ts)：`estimateTokens`/`estimateMessageTokens`/`getModelContextWindow`/`getAutoCompactThreshold`。
- [cost-tracker.ts](file:///Users/a0000/Desktop/myterminal/src/subagent/cost-tracker.ts)：`CostTracker`（setModel 结算旧定价），`resolvePricing`（精确→前缀→provider 估算→默认 gpt-4o）。
- [grep-utils.ts](file:///Users/a0000/Desktop/myterminal/src/subagent/grep-utils.ts)：`includePatternToRegex`/`matchInFiles`（IO 解耦）/`createGrep`。
- [tui-bridge.ts](file:///Users/a0000/Desktop/myterminal/src/subagent/tui-bridge.ts)：`subagentEvents` EventEmitter + 14 种 AG-UI 事件 + `emitAgUi`。
- [context.ts](file:///Users/a0000/Desktop/myterminal/src/subagent/context.ts)：`SubagentContext`（收敛 6 项）+ `createSubagentContext`/`defaultContext`。

### 6.6 TUI 系统（src/tui）

#### 整体架构
- [index.tsx](file:///Users/a0000/Desktop/myterminal/src/tui/index.tsx)：渲染入口。`runSetupTui`/`runWorkspaceChooserTui`/`runChoiceTui`/`class MyTerminalTui`（`run()` 创建 controller + 挂载 `<App>`）；`createRenderer`/`renderWithKeymap`。
- [App.tsx](file:///Users/a0000/Desktop/myterminal/src/tui/App.tsx)：核心组件。双速 tick（150ms 比对 `renderRevision()` 触发 refresh，1s `tickReminders`）；Ask 表单机制；`handleInputSubmit`→`routeCommand`；五层键盘路由；子代理审查队列 2s 轮询。
- [state.ts](file:///Users/a0000/Desktop/myterminal/src/tui/state.ts)：`TABS`（9 页签）、`class TuiController`（`snapshot`/`renderRevision`/`refreshDiff`/`createSession`/`sessionAction`/`sendMessage`/`editSettings`/`rotateCredentials`/`shutdown` 等）、`phaseColor`/`presenceColor`/`hiddenAppsUrl`。
- [contracts.ts](file:///Users/a0000/Desktop/myterminal/src/tui/contracts.ts)：`Detail`/`FormQuestion`/`Ask`/`RuntimeReconfigure` 类型。
- [controller-logic.ts](file:///Users/a0000/Desktop/myterminal/src/tui/controller-logic.ts)：纯函数层。`parseSelectedFields`/`buildSettingsQuestions`/`resolveSettingsAnswers`/`buildChildTaskPackage`/`sessionActionOptions`，`SETTINGS_FIELDS` 11 字段注册表。
...
| 文档 | 路径 |
|---|---|
| 架构与所有权 | [docs/architecture.md](file:///Users/a0000/Desktop/myterminal/docs/architecture.md) |
...
| AI 编码规则 | [AI_RULES.md](file:///Users/a0000/Desktop/myterminal/AI_RULES.md) |
| 开发约束 | [DEV_CONSTRAINTS.md](file:///Users/a0000/Desktop/myterminal/DEV_CONSTRAINTS.md) |
| 贡献指南 | [CONTRIBUTING.md](file:///Users/a0000/Desktop/myterminal/CONTRIBUTING.md) |
| 安全披露 | [SECURITY.md](file:///Users/a0000/Desktop/myterminal/SECURITY.md) |
