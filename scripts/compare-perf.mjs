...
// CI 通过 setup-bun 注入 ~/.bun/bin 到 PATH；本地与 CI 一致用绝对路径
...
const current = JSON.parse(out.slice(jsonStart));
...
const checks = [
  ...
  // issue63: context - 时间指标 (主要：~76ms 基准)
