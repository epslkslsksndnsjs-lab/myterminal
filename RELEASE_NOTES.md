...
- **Built-in adaptive-guard skill**. The system ships with a hardcoded `adaptive-guard` skill that provides a 7-category error recovery playbook (rate limit, turn interruption, timeout, identity staleness, network errors, context overflow, response size). It appears in the skill list automatically and loads when no user-authored override exists — user files always take priority.
...
- **内建 adaptive-guard skill**。系统自带硬编码 `adaptive-guard` skill，提供 7 类错误恢复操作手册（速率限制、turn 中断、超时、identity 失效、网络错误、上下文溢出、返回过大）。无用户文件时自动出现；用户同名文件始终优先。
