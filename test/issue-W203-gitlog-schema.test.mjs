// ADR-0051 增补-04 (#103)：git_log L3 豁免（#92 检查点裁决 A，用户拍板）
//
// 背景：真模型 20 样本评测 git_log×2 全 Q5 挂——答案字段被 Q5 剥光，结构性退化为
...
// （registry）；运行时探测走 MyTerminalRuntime actions 通道（myterminal.test.mjs 手法）。
...
function injectFake({ ready = true, object = null, throwing = false } = {}) {
  resetL3Adapter();
  let calls = 0;
  let lastReq = null;
  const adapter = {
  ...
  return { callCount: () => calls, getLastReq: () => lastReq };
}
...
function gitLogStructuredObject() {
  return {
    exitCode: 0,
    commits: [
...
// ───────────────────────────────────────────────────────────
// AC2/AC3：L3 永不调用（adapter 就绪 / 不可用 / 抛错）
// ───────────────────────────────────────────────────────────

test('W2-03-AC2: L3 永不调用 — adapter 就绪且结构化返回合法 → 仍不调模型，结果 L1 denoise', async () => {
  const { callCount, getLastReq } = injectFake({ object: { ...gitLogStructuredObject(), ghost: 'x' } });
  ...
  assertDenoisedResult(shaped, 'a1b2c3d fix typo\nf6e5d4c feat: add thing', getRecord);
});

test('W2-03-AC3: adapter 状态轮换（不可用 / 成功 / 抛错）→ 一律不调模型，L1 denoise（无失败矩阵）', async () => {
  for (const [label, opts] of [
    ['不可用', { ready: false }],
    ['成功', { ready: true, object: gitLogStructuredObject() }],
    ['抛错', { ready: true, throwing: true }],
  ]) {
    const { callCount, getLastReq } = injectFake(opts);
    const { ctx, getRecord } = makeCtx();
    const shaped = await shapeToolResponse(makeResponse('git_log', gitLogRaw()), ctx);

    assert.equal(callCount(), 0, `${label}：L3 永不调用`);
    assert.equal(getLastReq(), null, `${label}：adapter 不被咨询`);
    assertDenoisedResult(shaped, 'a1b2c3d fix typo\nf6e5d4c feat: add thing', getRecord, `${label}：`);
  }
});

// ───────────────────────────────────────────────────────────
// AC4：运行时探测 — actions 通道真实调用 git_log（临时 git 仓库）
// ───────────────────────────────────────────────────────────
...
test('W2-03-AC4: 运行时探测 — 真实 git_log 调用（fake adapter 注入）→ L3 永不调用，L1 denoise 数据保全', async () => {
  const dirs = tempGitWorkspace();
  // 即便 fake adapter 返回合法结构化抽取（旧 L3 会替换 stdout），豁免后也必须不调模型
  const { callCount, getLastReq } = injectFake({ object: gitLogStructuredObject() });
