...
- subagent_status({taskId}) polls progress. Returns {status, tasks, cost:{inputTokens,outputTokens,cacheReadTokens}, result}. Completed subagents are idempotent (re-reading returns the same result within a 1-hour cleanup window). NOT_FOUND means the subagent was cleaned up.
