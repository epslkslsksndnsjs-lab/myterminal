...
| `components.schemas ... is not an object` | 使用了旧版或被修改的 schema。MyTerminal 0.1.1 返回具体对象；从运行中的服务重新导入。 |
